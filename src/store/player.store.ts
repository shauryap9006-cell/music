...
import { buildPosterFallbackUrl, getPoster } from "@/frontend/lib/poster";
import { hashToColor, shuffleArray } from "@/frontend/lib/utils";
        ...
        try {
          ...
          preloadedSongs = await Promise.all(
            preloaded.map(async (song) => {
              const shouldEnrich =
                song.folder === "Preview Songs" && song.audio_url.startsWith("/preview-songs/");
              ...
              return normalizeSong({
                ...
                artUrl: metadata?.coverUrl ?? poster ?? undefined,
                source: "preloaded",
                folder: song.folder ?? "Preview Songs"
              });
            })
          );
        } catch {
